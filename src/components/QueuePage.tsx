...
export default function QueuePage({ queue, currentTrack, isPlaying, onRemove, onClear, onPlayIndex }: Props) {
  ...
  return (
    <div className="space-y-6 animate-slide-up">
      ...
      {currentTrack && (
        <div className="glass-md rounded-3xl overflow-hidden glow-indigo">
          <div className="h-1 bg-gradient-to-r from-indigo-500 via-purple-500 to-fuchsia-500" />
          <div className="p-6">
            <div className="text-[10px] text-indigo-500 font-bold uppercase tracking-[.15em] mb-4">Сейчас играет</div>
            <div className="flex items-center gap-5">
              <div className="w-14 h-14 rounded-xl bg-gradient-to-br from-indigo-100 to-purple-100 border border-indigo-200/50 flex items-center justify-center flex-shrink-0">
                {isPlaying ? (
                  <div className="flex items-end gap-[2px] h-5">
                    {[12, 18, 8, 16, 14].map((h, i) => (
                      <div key={i} className="waveform-bar" style={{ '--h': `${h}px`, '--d': `${0.3 + i * 0.08}s`, '--delay': `${i * 0.04}s` } as React.CSSProperties} />
                    ))}
                  </div>
                ) : (
                  <IcMusic className="w-6 h-6 text-indigo-400" />
                )}
              </div>
      ...
      {queue.length === 0 ? (
        <div className="text-center py-20">
          <div className="w-16 h-16 rounded-2xl glass mx-auto mb-5 flex items-center justify-center">
            <IcQueue className="w-7 h-7 text-slate-300" />
          </div>
        ...
        <div className="space-y-2">
          {queue.map((track, i) => (
            <div key={`${track.id}-${i}`} className="glass rounded-2xl px-5 py-4 flex items-center gap-4 hover:bg-white/70 transition-all group">
              ...
              <button onClick={() => onPlayIndex(i)} className="w-10 h-10 rounded-xl bg-slate-50 border border-slate-200 flex items-center justify-center opacity-50 group-hover:opacity-100 group-hover:bg-indigo-50 group-hover:border-indigo-200 transition-all flex-shrink-0">
              ...
              <button onClick={() => onRemove(i)} className="p-2 text-slate-200 hover:text-rose-500 transition-colors rounded-lg hover:bg-rose-50">
