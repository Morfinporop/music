...
export default function UploadPage({ onAdd }: Props) {
  ...
  const handleFiles = (fileList: FileList | null) => {
    ...
    arr.forEach(f => {
      ...
      onAdd(createTrack({
        ...
        artist: parts.length > 1 ? parts[0].trim() : 'Неизвестен',
  ...
  return (
    <div className="space-y-6 animate-slide-up">
      <div>
        <h1 className="text-3xl font-extrabold text-slate-800 tracking-tight">Добавить трек</h1>
        <p className="text-sm text-slate-400 mt-2">Добавь музыку в библиотеку</p>
      </div>

      <div className="flex gap-1 p-1 bg-white/60 rounded-xl border border-slate-200/50 w-fit">
        <button onClick={() => setMode('manual')} className={`px-5 py-2.5 rounded-lg text-xs font-bold transition-all flex items-center gap-2 ${mode === 'manual' ? 'bg-white text-slate-700 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}>
          <IcPlus className="w-3.5 h-3.5" />Вручную
        </button>
        <button onClick={() => setMode('upload')} className={`px-5 py-2.5 rounded-lg text-xs font-bold transition-all flex items-center gap-2 ${mode === 'upload' ? 'bg-white text-slate-700 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}>
          <IcUpload className="w-3.5 h-3.5" />Файл
        </button>
      </div>
      ...
      {mode === 'manual' ? (
        <div className="glass-md rounded-3xl p-8 max-w-lg space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="text-[11px] text-slate-500 font-bold block mb-2">Источник</label>
        ...
        <>
          ...
          >
            ...
            <div className="w-14 h-14 rounded-2xl glass mx-auto mb-5 flex items-center justify-center">
              <IcUpload className="w-6 h-6 text-slate-400" />
            </div>
            <p className="text-sm text-slate-500 font-medium mb-1">Перетащи файлы или нажми</p>
            <p className="text-xs text-slate-300">MP3, WAV, OGG, FLAC, M4A</p>
          </div>
                ...
                {files.map((f, i) => (
                  <div key={i} className="flex items-center gap-3 py-2">
                    <div className="w-7 h-7 rounded-lg bg-emerald-50 border border-emerald-200 flex items-center justify-center flex-shrink-0">
                      <IcCheck className="w-3.5 h-3.5 text-emerald-500" />
                    </div>
                    <span className="text-sm text-slate-500 truncate flex-1">{f.name}</span>
                    <span className="text-[10px] text-slate-300 mono">{(f.size / 1048576).toFixed(1)} MB</span>
                  </div>
                ))}
              </div>
            </div>
...
function Field({ label, value, onChange, placeholder }: { label: string; value: string; onChange: (v: string) => void; placeholder: string }) {
  return (
    <div>
      <label className="text-[11px] text-slate-500 font-bold block mb-2">{label}</label>
